...
import { User, UserRole } from "@/entities/users/users.type";
...
export function UpdateUserForm({ user }: Props) {
    const { register } = useUpdateUserForm(user);
    
    const updateUserWithId = updateUserAction.bind(null, user.id);
    const [state, formAction, isPending] = useActionState(updateUserWithId, { errors: [] });
